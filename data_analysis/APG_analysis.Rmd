...
### Pairwise Comparison: Learnable Pre-Exposure vs. No Pre-Exposure {.tabset}

Investigate the relationship between the Learnable Pre-Exposure condition and the No Pre-Exposure condition, using all of the data collected across Experiments 1-3.
...
m <- glmer(isRight~conditionC+(1|participantCode),data=d,family=binomial,glmerControl(optimizer="bobyqa"))
summary(m)
confint(m, method="Wald")
```
...
```{r}
m <- glmer(isRight~conditionC+(1|participantCode)+(1+conditionC|stimulus),data=d,family=binomial,glmerControl(optimizer="bobyqa"))
summary(m)
confint(m, method="Wald")[5:6,]
```
...
```{r}
m <- glmer(isRight~conditionC+(1|participantCode),data=subset(d,testType=="familiarX"),family=binomial,glmerControl(optimizer="bobyqa"))
summary(m)
confint(m, method="Wald")
```
...
```{r}
m <- glmer(isRight~conditionC+(1|participantCode)+(1+conditionC|stimulus),data=subset(d,testType=="familiarX"),family=binomial,glmerControl(optimizer="bobyqa"))
summary(m)
confint(m, method="Wald")[5:6,]
```
...
```{r}
m <- glmer(isRight~conditionC+(1|participantCode),data=subset(d,testType=="novelX"),family=binomial,glmerControl(optimizer="bobyqa"))
summary(m)
confint(m, method="Wald")
```
...
```{r}
m <- glmer(isRight~conditionC+(1|participantCode)+(1+conditionC|stimulus),data=subset(d,testType=="novelX"),family=binomial,glmerControl(optimizer="bobyqa"))
summary(m)
confint(m, method="Wald")[5:6,]
```
...
m <- lm(dprime~conditionC,data=subj_overall)
summary(m)
Anova(m, type="III")
```
...
m <- lm(dprime~conditionC,data=filter(subj_testType, testType=="familiarX"))
summary(m)
Anova(m, type="III")
```
...
```{r}
m <- lm(dprime~conditionC,data=filter(subj_testType, testType=="novelX"))
summary(m)
Anova(m, type="III")
```

### Pairwise Comparison: Learnable Pre-Exposure vs. Non-Learnable Pre-Exposure {.tabset}

Investigate the relationship between the Learnable Pre-Exposure condition and the Non-Learnable Pre-Exposure condition, using all of the data collected across Experiments 1-3.
...
### Pairwise Comparison: Learnable Pre-Exposure vs. Unstructured Pre-Exposure {.tabset}

Investigate the relationship between the Learnable Pre-Exposure condition and the Unstructurd Pre-Exposure condition, using all of the data collected across Experiments 1-3.
